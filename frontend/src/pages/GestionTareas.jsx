...
const GestionTareas = () => {
  ...
  const [taskTab, setTaskTab] = useState('pendientes'); // pendientes, resueltas
      ...
      {loading ? (
        ...
        <div className="animate-fade-in">
          ...
          {activeTab === 'tareas' && (() => {
            ...
            const resolvedTasks = tasks.filter(t => t.status === 'COMPLETED' && new Date(t.updatedAt || t.createdAt) >= oneMonthAgo);
            const currentTasks = taskTab === 'pendientes' ? pendingTasks : resolvedTasks;
            return (
            <div className="space-y-4">
              ...
              <div className="flex gap-2 border-b border-slate-200 dark:border-slate-700 mb-4">
                <button
                  ...
                  className={`pb-3 px-3 text-sm font-bold transition-colors relative flex items-center gap-2 ${
                    taskTab === 'pendientes' ? 'text-blue-600 border-b-2 border-blue-600' : 'text-slate-500 hover:text-slate-700'
                  }`}
                >
                  Pendientes
                  <span className="bg-amber-100 text-amber-700 py-0.5 px-2 rounded-full text-xs">{pendingTasks.length}</span>
                </button>
                <button
                  onClick={() => setTaskTab('resueltas')}
                  className={`pb-3 px-3 text-sm font-bold transition-colors relative flex items-center gap-2 ${
                    taskTab === 'resueltas' ? 'text-emerald-600 border-b-2 border-emerald-600' : 'text-slate-500 hover:text-slate-700'
                  }`}
                >
                  Resueltas
                  <span className="bg-emerald-100 text-emerald-700 py-0.5 px-2 rounded-full text-xs">{resolvedTasks.length}</span>
                </button>
              </div>
                  ...
                  <div key={t.id} className="glass-panel p-4 rounded-xl flex justify-between items-center">
                    ...
                    <div className="flex items-center gap-4">
                      <span className={`px-2 py-1 rounded-full text-[10px] font-black uppercase ${t.status === 'COMPLETED' ? 'bg-emerald-100 text-emerald-700' : 'bg-blue-100 text-blue-700'}`}>
                        {t.status === 'COMPLETED' ? 'Resuelta' : 'Pendiente'}
                      </span>
                      {t.status !== 'COMPLETED' && (
                        user?.role === 'admin' ? (
                ...
                {currentTasks.length === 0 && (
                  <div className="glass-panel p-12 rounded-2xl text-center flex flex-col items-center gap-3">
                    ...
                    <h4 className="text-lg font-bold text-slate-700 dark:text-slate-300">
                      {taskTab === 'pendientes' ? 'Sin tareas pendientes' : 'Sin resueltas recientes'}
                    </h4>
        ...
        <div className="fixed inset-0 z-50 flex items-start justify-center pt-10 bg-slate-900/50 backdrop-blur-sm p-4 overflow-y-auto">
          <div className="bg-white dark:bg-slate-900 w-full max-w-md rounded-2xl shadow-2xl p-6">
            ...
            <form onSubmit={handleAssignTask} className="space-y-4">
              ...
              <div className="flex gap-3 pt-2">
                <button type="button" disabled={submittingTask} onClick={() => setShowTaskModal(false)} className="flex-1 px-4 py-2 rounded-xl border font-bold dark:text-white disabled:opacity-50">Cancelar</button>
                <button 
                  type="submit" 
                  disabled={submittingTask} 
                  className="flex-1 bg-blue-600 hover:bg-blue-700 disabled:opacity-50 text-white px-4 py-2 rounded-xl font-bold flex items-center justify-center gap-2"
                >
                  {submittingTask ? (
                    <>
